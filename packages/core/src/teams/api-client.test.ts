...
import { registerTeamsRuntime, type TeamsRuntime } from "../runtime";
import { TeamsApiClient } from "./api-client";

const cacheImageFile = vi.fn();
const filePathToAssetUrl = vi.fn((filePath: string) => `asset://${filePath}`);
...
beforeEach(() => {
  ...
  getCachedImageFile.mockReset();
  hasCachedImageFile.mockReset();
  const runtime: TeamsRuntime = {
    fetch: globalThis.fetch,
    extractTokens: vi.fn().mockResolvedValue([]),
    getAuthToken: vi.fn().mockResolvedValue(null),
    getAvailableAccounts: vi.fn().mockResolvedValue([]),
    getCachedPresence: vi.fn().mockResolvedValue([]),
    cacheImageFile,
    filePathToAssetUrl,
    getCachedImageFile: getCachedImageFile.mockResolvedValue(null),
    hasCachedImageFile: hasCachedImageFile.mockResolvedValue(false),
  };
