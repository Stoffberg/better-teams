...
import { getTeamsRuntime, type TeamsRuntime } from "../runtime";
import {
...
export class TeamsApiClient {
  ...
  private readonly runtime: TeamsRuntime;
  ...
  constructor(tenantId?: string, options?: TeamsApiClientOptions) {
    ...
    this.runtime = options?.runtime ?? getTeamsRuntime();
    this.httpFetch =
      options?.fetchImpl ??
      ((input, init) => this.runtime.fetch(input as RequestInfo, init));
  ...
  async initialize(): Promise<void> {
    this.authToken = await this.runtime.getAuthToken(this.tenantId);
    if (!this.authToken) {
      throw new Error(
  ...
  > {
    const tokens = await getTeamsRuntime().extractTokens();
  ...
  async fetchAuthenticatedImageSrc(imageUrl: string): Promise<string | null> {
    ...
    for (const headers of headerSets) {
      ...
      return this.runtime.filePathToAssetUrl(filePath);
    }
    return null;
  }
